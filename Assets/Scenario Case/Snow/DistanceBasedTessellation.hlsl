...
TessllationFactors DistanceBasedTess(float4 v0,float4 v1,float4 v2,float minDist,float maxDist, float tess)
{
    ...
    f.y = CalDistanceTessFactor(v0,minDist,maxDist,tess);
    f.z = CalDistanceTessFactor(v0,minDist,maxDist,tess);
...
[UNITY_domain("tri")]
Varyings DistanceBasedTessellationDomain(TessllationFactors factors,OutputPatch<ControlPoint,3> patch , float3 barycentriCoordinates : SV_DomainLocation)
{
    Attributes v;
